...
func main() {
	...
	best.Describe("異常系テスト", func() {
		best.It("無効なコマンドにはエラーメッセージが含まれるべき", func(ctx *best.TestContext) {
			...
			// PNXはCommandOutputパケットで "commands.generic.unknown" を返す
			agent.Command("/this_command_does_not_exist")
			agent.Expect().CommandOutput().ToContain("unknown", 3*time.Second)
		})
	...
	best.Describe("マルチエージェント連携", func() {
		...
		// 注意: グローバルのagent(BestTestBot)と名前が重複しないようにする
		best.BeforeAll(func(ctx *best.TestContext) {
			agent1 = best.CreateAgent("MultiAgent1")
			if err := agent1.Connect(); err != nil {
				panic(err)
			}
			time.Sleep(2 * time.Second)

			agent2 = best.CreateAgent("MultiAgent2")
		...
		best.It("両方のエージェントがコマンドを実行できるべき", func(ctx *best.TestContext) {
			...
			// 異なるコマンドを順番に実行する
			// PNXはTextPacketでレスポンスを返す
			agent1.Command("/help")
			agent1.Expect().Chat().ToReceive("/ban", 3*time.Second, nil)

			// /listはプレイヤー名のリストを返す
			agent2.Command("/list")
			agent2.Expect().Chat().ToReceive("MultiAgent", 3*time.Second, nil)
		})
